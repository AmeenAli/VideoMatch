...
class PrecompDataset(data.Dataset):
  ...
  def __init__(self, data_path, data_split, vocab, opt):
    self.vocab = vocab
    json_path = osp.join(this_dir, 'YouCook2', 'yc2', '{}.json'.format('yc2_annotations_trainval') )
    self.data_split = data_split
    # Captions
    self.jsondict = jsonmod.load(open(json_path, 'r'))
    self.ann_id = {}
    counter = 0
    for i, keys in enumerate(self.jsondict['database'].keys()):
      if data_split == 'train' and self.jsondict['database'][keys]['subset'] == 'validation':
       continue
      if data_split == 'val' and self.jsondict['database'][keys]['subset'] == 'training':
      
       continue
      self.ann_id[counter] = keys
      counter = counter + 1
  ...
  def img_cap_feat_combine(self, video_feat, caption_feat, cur_vid):
    #### Videos ####
    frame_duration = self.jsondict['database'][cur_vid]['duration']
    segment_number = len(self.jsondict['database'][cur_vid]['annotations'])
    video_feat_len = video_feat.shape[0]
    clips = []
    for seg_id in range(segment_number):
      picked_segment = self.jsondict['database'][cur_vid]['annotations'][seg_id]['segment']
    ...
    for cap in caption_feat:
      tokens_cap = nltk.tokenize.word_tokenize(cap['sentence'].lower())
      _cap = []
...
#      exit(-1)
  ...
  def __getitem__(self, index):
    ...
    image_data = np.load('/media/data1/ameenali/clean/nips_2020/data/youcook_precomp/vids/'+ cur_vid + '_resnet' + '.npy').squeeze()
    image = torch.Tensor(image_data)
    caption_json = self.jsondict['database'][cur_vid]['annotations']
